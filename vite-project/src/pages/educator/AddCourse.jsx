...
const AddCourse = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    alert("Course added successfully! (Simulated)")
  }
  ...
  return (
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0'>
      <form onSubmit={handleSubmit} className='flex flex-col gap-4 ma-w-md w-full text-gray-500'>
        <div className='flex flex-col gap-1'>
          <p>Course Title</p>
          <input className='outline-none md:py-2.5 py-2 px-3 rounded border border-gray-500' type="text"
            onChange={e => setCourseTitle(e.target.value)} placeholder='Type here' value={courseTitle} required />
        </div>
        <div className='flex flex-col gap-1'>
        ...
        <div className='flex items-center justify-between flex-wrap'>
          ...
          <div className='flex flex-col gap-1'>
            <p>Course Price</p>
            <input className='outline-none md:py-2.5 py-2 px-3  rounded border border-gray-500' type="number"
              onChange={e => setCoursePrice(e.target.value)} placeholder='0' value={coursePrice} required />
          </div>
          ...
          <div className='flex md:flex-row flex-col items-center gap-3'>
            ...
            <label htmlFor="thumbnailImage" className='flex items-center gap-3'>
              ...
              <img src={image ? URL.createObjectURL(image) : assets.upload_area_icon} alt="" className='max-h-10' />
            </label>
          </div>
        </div>
        ...
        <div className='flex flex-col gap-1'>
          <p>Discount %</p>
          <input className='outline-none md:py-2.5 py-2 px-3 w-50 rounded border border-gray-500' type="number"
            onChange={e => setDiscount(e.target.value)} placeholder='0' min={0} max={100} value={discount} required />
        </div>
          ...
          {showPopup && (
            <div className="fixed inset-0 flex items-center justify-center bg-gray-200 bg-opacity-50 z-50">
              <div className='bg-white text-gray-700 p-4 rounded relative w-full max-w-80 z-50'>
                ...
                <img src={assets.cross_icon} alt="" className='absolute top-4 right-4 w-4 cursor-pointer'
        ...
        <button type='submit' className='bg-black text-white w-max py-2.5 px-8 rounded my-4 cursor-pointer'>Add Course</button>
